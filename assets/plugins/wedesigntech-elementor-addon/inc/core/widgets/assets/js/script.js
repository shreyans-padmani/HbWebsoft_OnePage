(function ($) {
    ...
    const wdtWidgetsOptionsHandler = function($scope) {
        ...
        inviewInstance.init();
		const darkModeInstance = new wdtWidgetsDarkModeHandlerInit($scope);
	...
	const wdtWidgetsDarkModeHandlerInit = function($scope) {
		const $self   = this,
			$window   = $(window),	
			$widgetId = $scope.data('id'),
			$editMode = Boolean(elementorFrontend.isEditMode());
		...
		let $darkModeClass = 'wdt-dark-bg';

		$self.init = function() {
			$self.darkModeInit();
		}
		$self.darkModeInit = function() {

			$darkModeSettings = $scope.data('settings') || false;
			$darkModeSettings = (false !== $darkModeSettings) ? $darkModeSettings : false;
		

			if(!$darkModeSettings || !$darkModeSettings['wdt_dark_mode']) {
				return false;
			}

			$darkModeEnabled = Boolean($darkModeSettings['wdt_dark_mode']);

			if($darkModeEnabled) {
				$scope.addClass($darkModeClass);
			} else {
				$scope.removeClass($darkModeClass);
			}
